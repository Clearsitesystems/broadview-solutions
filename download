...
import { Lead, LeadStatus } from '@/hooks/useCrmStorage';
...
const blank: Omit<Lead, 'id' | 'createdAt'> = {
  name: '', email: '', phone: '', address: '', propertyType: 'residential',
  serviceInterest: 'lawn-maintenance', message: '', preferredContact: 'email',
  status: 'new', source: 'Manual Entry', assignedTo: '', notes: '',
};

export default function LeadModal({ lead, currentUser, onSave, onClose }: Props) {
  const [form, setForm] = useState<Omit<Lead, 'id' | 'createdAt'>>(blank);
      ...
      <div className="bg-[#1A2424] border border-[#8A9BA8]/20 rounded-xl w-full max-w-2xl">
        ...
        <div className="p-6 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-[#8A9BA8] mb-1.5">Full Name *</label>
            ...
            <div>
              <label className="block text-xs font-medium text-[#8A9BA8] mb-1.5">Service Interest</label>
              <select value={form.serviceInterest} onChange={(e) => set('serviceInterest', e.target.value)} className="w-full px-3 py-2.5 bg-[#1C2B2B] border border-[#8A9BA8]/25 text-white rounded-lg text-sm outline-none focus:ring-2 focus:ring-[#2D5016] cursor-pointer">
            ...
            <div>
              ...
              <select value={form.status} onChange={(e) => set('status', e.target.value as LeadStatus)} className="w-full px-3 py-2.5 bg-[#1C2B2B] border border-[#8A9BA8]/25 text-white rounded-lg text-sm outline-none focus:ring-2 focus:ring-[#2D5016] cursor-pointer">
                {statusOptions.map((s) => (
            ...
            <div>
              <label className="block text-xs font-medium text-[#8A9BA8] mb-1.5">Source</label>
              <select value={form.source} onChange={(e) => set('source', e.target.value)} className="w-full px-3 py-2.5 bg-[#1C2B2B] border border-[#8A9BA8]/25 text-white rounded-lg text-sm outline-none focus:ring-2 focus:ring-[#2D5016] cursor-pointer">
            ...
            <div>
              <label className="block text-xs font-medium text-[#8A9BA8] mb-1.5">Preferred Contact</label>
              <select value={form.preferredContact} onChange={(e) => set('preferredContact', e.target.value)} className="w-full px-3 py-2.5 bg-[#1C2B2B] border border-[#8A9BA8]/25 text-white rounded-lg text-sm outline-none focus:ring-2 focus:ring-[#2D5016] cursor-pointer">
          ...
          <div>
            <label className="block text-xs font-medium text-[#8A9BA8] mb-1.5">Message / Details</label>
            <textarea value={form.message} onChange={(e) => set('message', e.target.value)} rows={3} maxLength={500} className="w-full px-3 py-2.5 bg-[#1C2B2B] border border-[#8A9BA8]/25 text-white rounded-lg text-sm outline-none focus:ring-2 focus:ring-[#2D5016] resize-none" />
          </div>

          <div>
            <label className="block text-xs font-medium text-[#8A9BA8] mb-1.5">Internal Notes</label>
